...
PROPMPTS = {
    ...
    "writer": """
        ...
        "checker": """
            ...
            2. **清晰度**：回复是否清晰易懂，避免了模糊或歧义？确保语句简洁且逻辑清晰。特别是，参考信息是否在合适的地方使用，避免信息的冗余和重复。
            3. **礼貌性**：回复是否符合基本的礼貌规范，语气是否正式且友善？请检查是否有不必要的口语或过于简略的表达。
            4. **格式**：回复的格式是否规范，段落分明，语法正确，标点恰当？检查邮件中参考信息的插入方式，是否影响邮件格式的整洁性。
            5. **相关性**：回复是否与原邮件的内容相关，是否对邮件中的问题做出了有意义的回应？特别是参考信息的插入是否合理，是否有帮助于申请人了解学校的具体情况。
